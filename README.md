# Práctica: Fundamentos de Estructura y Contenido HTML

## Descripción de la Actividad
Esta actividad tiene como objetivo aprender a estructurar contenido básico en HTML utilizando etiquetas esenciales para texto, enlaces, imágenes y secciones semánticas, sin depender de CSS o JavaScript.

Se ha desarrollado un archivo `index.html` que funciona como un menú principal, el cual contiene enlaces (mediante la etiqueta `<a>`) a cada uno de los ejercicios solicitados.

## Ejercicios Desarrollados
1. **Ejercicio 2: Párrafos y Formato Básico.** Uso de etiquetas `<p>`, `<strong>`, `<em>` y `<br>`.
2. **Ejercicio 3: Enlaces Simples.** Creación de enlaces externos y anclas usando `<a>`, junto con los atributos `target="_blank"` y `target="_self"`.
3. **Ejercicio 4: Imágenes Básicas.** Inserción de imágenes con `<img>`, usando los atributos `src`, `alt` y `width`.
4. **Ejercicio 5: Combinando Texto y Enlaces.** Integración de texto, imágenes dentro de párrafos y enlaces absolutos.
5. **Ejercicio 6: Secciones Semánticas Básicas.** Uso de etiquetas estructurales como `<header>`, `<section>` y `<footer>`.

## Estructura del Proyecto
* `index.html`: Menú principal de navegación hacia los ejercicios.
* `ejercicio2.html`
* `ejercicio3.html`
* `ejercicio4.html`
* `ejercicio5.html`
* `ejercicio6.html`
* `img/`: Carpeta que contiene los recursos gráficos (ej. `paisaje.jpg`, `viaje.jpg`).

## Requisitos e Instrucciones de Ejecución
* Tener instalado XAMPP.
* Iniciar el servidor Apache desde el panel de control de XAMPP.
* Colocar la carpeta del proyecto (ej. `prograweb`) dentro del directorio `htdocs` de XAMPP.
* Abrir el navegador web y acceder a: `http://localhost/prograweb/index.html`